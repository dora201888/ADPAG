---
title: "ADPAG: An Open-Source R/R-Markdown Package for Accelerometer Data Processing after Running GGIR"
author: "Wei Guo"
date: '`r format(Sys.Date(), "%B %d, %Y")`'
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{ADPAG}
...
#
**Figure 1: Overview of main steps and output in ADPAG workflow.**
...
```{R,eval=FALSE}
install.packages("ADPAG", dependencies = TRUE)
```
...
## Create a template shell script of ADPAG 
...
The function will create a template shell script of ADPAG in the current directory, names as *STUDYNAME_module0.maincall.R*.
...
filename2id.2<-function(x) {
  d<-read.csv("./ADPAG/inst/extdata/example/filename2id.csv",head=1,stringsAsFactors=F)
...
adpag.shell<-function(mode,filename2id=NULL){ 

library(ADPAG) 
packageVersion("ADPAG")
#  ?adpag.maincall  # run help to see all argumengts 
...
rmDup=FALSE   # keep all subjects in ADPAG
...
This user-defined function will change the filename of the raw accelerometer file to the short ID. For example, the first example change "0002__026907_2016-03-11 13-05-59.bin" to new ID of "0002". If you prefer to define new ID by other way, you could create a .CSV file including "filename" and "newID" at least and then defined this function as the second example. The new variable of "newID", included in the output files, could be used as the key ID in the summary report of ADPAG and be used to define the duplicate samples as well. 


### 2. Parameters of adpag.maincall() 
...
The ADPAG package not only simply transform/merge the activity and sleep data, but it also can do some prelimary data analysis such as principle componet analysis and feature extraction. Therefore, the basic data clean will be processed first as follows,     
...
feaFN<-system.file("template", "features.dictionary.xlsx", package = "ADPAG")   
...
d1<-read.xlsx("ADPAG.output.description.xlsx",sheetName="output.format")  

cd /data/guow4/project0/GGIR/postGGIR/postGGIR_compile/v2/ADPAG/vignettes     
R -e "rmarkdown::render('ADPAG.Rmd'   )"  
```


# Citing ADPAG  
   +  Guo W., Leroux A., Shou S., Cui L., Kang S.,  Strippoli M.P., Preisig M., Zipunnikov V., Merikangas K. (2022) ADPAG: An Open-Source R/R-Markdown Package for Accelerometer Data Processing after Running GGIR. (Manuscript)  
...
# Websites  
   +  ADPAG download: https://CRAN.R-project.org/package=ADPAG 

   +  ADPAG vignette: https://cran.r-project.org/web/packages/ADPAG/vignettes/ADPAG.html

   +  Example data and documents: https://github.com/dora201888/ADPAG
